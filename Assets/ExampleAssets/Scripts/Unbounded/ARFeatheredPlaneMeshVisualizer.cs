...
namespace PolySpatial.Template
{
    ...
    {
        static readonly int k_ShortestUVMapping = Shader.PropertyToID("_ShortestUVMapping");

        static readonly List<Vector3> k_FeatheringUVs = new();
        static readonly List<Vector3> k_Vertices = new();
        ...
        void GenerateBoundaryUVs(Mesh mesh)
        {
            ...
            k_FeatheringUVs.Clear();
            if (k_FeatheringUVs.Capacity < vertexCount) { k_FeatheringUVs.Capacity = vertexCount; }

            mesh.GetVertices(k_Vertices);

            var centerInPlaneSpace = k_Vertices[k_Vertices.Count - 1];
            ...
            {
                var vertexDist = Vector3.Distance(k_Vertices[i], centerInPlaneSpace);
                ...
                k_FeatheringUVs.Add(uv);
            }

            m_FeatheredPlaneMaterial.SetFloat(k_ShortestUVMapping, shortestUVMapping);

            // Add the center vertex UV
            uv.Set(0, 0, 0);
            k_FeatheringUVs.Add(uv);

            mesh.SetUVs(1, k_FeatheringUVs);
